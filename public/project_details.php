...
$query1="select * from project where project_id={$project_id}";
$query2="select u.user_name from user u,project p where p.user_id=u.user_id and p.project_id={$project_id}";
$query3="select * from donor where project_id={$project_id}";
$query4="select * from updates where project_id={$project_id}";
$query7="select * from comment where project_id={$project_id}";
...
$percent=$amount*100/$row1['project_funds'];
$result4=mysqli_query($conn,$query4);
$result6=mysqli_query($conn,$query3);
$result7=mysqli_query($conn,$query7);


if(isset($_SESSION['flag_excess_amt']))
{$flag_excess_amt=$_SESSION['flag_excess_amt'];
if($flag_excess_amt==1){
	echo '<script language="javascript">';
echo 'alert("Transaction successful, Details tracked by admin due to excessive amount")';
echo '</script>';
}
else if($flag_excess_amt==0){
		echo '<script language="javascript">';
echo 'alert("Transaction successfull")';
echo '</script>';
}
unset($_SESSION['flag_excess_amt']);
}
?>
<head>
